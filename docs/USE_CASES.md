...
> **Critical input rule.** All headword input is **Harvard-Kyoto (HK) ASCII romanization**, *not* Unicode Devanagari or Tamil script. The stored data is single-byte ASCII HK (e.g. `akAra`, with `%{...}` markup). Type romanized HK — the form does not accept native script. See [Harvard-Kyoto transliteration reference](#harvard-kyoto-transliteration-reference).
