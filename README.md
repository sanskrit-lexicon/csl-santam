...
4. `compute_where()` sets the dictionary scope (`id<4` for `all`, else `id=$dictnum`) and calls `where1()` per non-empty field. `where1()` splits each field's value on spaces (`preg_split('/ +/', …)`) and AND-joins the words. Match mode per field: `exact`/`prefix`/`suffix` route through a custom SQLite `regexp` UDF (`_sqliteRegexp` → `preg_match('/'.$pattern.'/i', …)`) with `\b` word-boundary anchors; `substring` uses native SQL `LIKE '%…%'`. Matching is case-insensitive by default via `lower(col)` / `strtolower` — unless the form's "Case-sensitive search" checkbox is set, in which case folding is skipped, `exact`/`prefix`/`suffix` route through a second `regexp_cs` UDF (no `/i` flag, called via function-call syntax since SQLite's `REGEXP` operator is hardwired to a function literally named `regexp`), and `substring` relies on `PRAGMA case_sensitive_like`.
...
> **Case-insensitivity caveat — opt-out available since `0.1.0`+Wave 2.** By default both the data column and the query term are lowercased before matching, so HK case distinctions collapse: `aTa` folds to `ata`, conflating retroflex `T` with dental `t`; `A` (long ā) with `a`; and the three sibilants `z`/`S`/`s` with each other. This remains the default, but the form's **"Case-sensitive search" checkbox** (`case_sensitive=1`) now opts out entirely, so power users can search on the exact HK letters instead of relying on spelling/context to disambiguate.
